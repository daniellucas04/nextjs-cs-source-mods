    ...
    <>
      {weaponList.map((item, idx) => (
        <div key={idx} className="w-[7em] transition">
          <h1 className="text-base text-60-dark dark:text-60-white font-bold w-max flex p-1 cursor-default">
            {item.category}
          </h1>
          <ul className="flex flex-col h-min gap-1 rounded-xl px-1">
            {item.weapons.map((weapon, idx) => (
              <Link
                href={``}
                className="text-sm ml-1 w-max text-60-dark dark:text-60-white hover:underline hover:text-10-white dark:hover:text-10-dark cursor-pointer"
                key={idx}
              >
                {weapon}
              </Link>
