import { server_side, knifes, gloves, weapons } from "./modsList";
...
export default function List() {
  ...
  function getModArray() {
    let modArray = ["server_side", "knifes", "gloves", "weapons"];

    for (let element in modArray) {
      if (item == modArray[element]) {
        return modArray[element];
      }
    }
  }

  function getModFromQuery() {
    const modsList = [server_side, knifes, gloves, weapons];
    const modElement = getModArray();
    console.log(JSON.parse(modElement));

    for (let element in modsList) {
      // if (modElement == modsList[element]) {
      //   setModItem(modsList[element]);
      // }
    }
